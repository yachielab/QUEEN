                ...
                if "note_dbrick" in feat.qualifiers:
                    if (gs == 0 or ge >= length) and (pos_s != 1 or pos_e != feat_length):
                        if gs == 0 and ge >= length:
                            ...
                            ax.arrow(x=ge, y=-1*y, dx=gs-ge, dy=0, width=0.8, head_width=0.8, head_length=hl, length_includes_head=True, color='k', fc=edgecolor, lw=0.0)
                            ax.arrow(x=ge-hl*margin1, y=-1*y, dx=gs-ge+margin1, dy=0, width=wd2, head_width=wd2, head_length=0, length_includes_head=True, color='k', fc=facecolor, lw=0.0)
                        else:
                            ax.arrow(x=ge, y=-1*y, dx=gs-ge, dy=0, width=0.8, head_width=0.8, head_length=hl, length_includes_head=True, color='k', fc=edgecolor, lw=0.0)
...
def visualize(brick, start=0, end=None, aspect=None, unvisible_types=["source"], visible_types=[], enlarge=1.0):
    ...
    ticks       = [x for p, x in zip(positions, list(range(0, len(sub_brick.seq)))) if p % (std/(2*int(enlarge))) == 0 or p == 1] 
    tick_labels = [str(p) for p, x in zip(positions, list(range(0, len(sub_brick.seq)))) if p % (std/(2*int(enlarge))) == 0 or  p == 1]
    ax.set_xticks(ticks) 
    ...
    """
    if head_length < 10:
        ax = fig.add_axes([0, -0.5/(max(y_list)+1), 1.0*len(brick.seq)/std, 0.01/(max(y_list)+1)])
        ax.set_xlim(0, len(brick.seq))
        ax.spines["top"].set_visible(False) 
        ax.spines["right"].set_visible(False) 
        ax.spines["left"].set_visible(False) 
        ax.spines["bottom"].set_visible(False) 
        #ax.tick_params(which="minor", length=0) 
        #ax.set_xticks([p+0.5for p in range(len(brick.seq))])
        #ax.set_xticklabels(brick.seq)
    """
