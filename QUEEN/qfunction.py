...
def cutdna(dna, *positions, crop=False, project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1):   
    ...
    if __direct == 1:
        ...
        for subdna in dnas:
            ...
            process_id = make_processid(subdna, building_history, process_id)
            add_history(subdna, [building_history, "positions: {}".format(",".join(list(map(str, new_positions_original)))) + "; num_products: {}".format(len(dnas)), ",".join([process_id] + originals)])
    else:
        for subdna in dnas:
...
def cropdna(dna, start=0, end=None, project=None, product=None, process_description=None, process_name=None, pd=None, pn=None, process_id=None, originals=[], __direct=1):
    project = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description
    ...
    if __direct == 1:   
        ...
        subdna._history_feature = history_feature
        process_id = make_processid(subdna, building_history, process_id)
        add_history(subdna, [building_history, "start: {}; end: {}".format(*new_positions), ",".join([process_id] + originals)]) 
    else:
        subdna.__dict__["_product_id"] = dna._product_id if "_product_id" in dna.__dict__ else dna._unique_id
...
def joindna(*dnas, topology="linear", project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1):
    project             = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description

    if process_description is None:
        process_description = dnas[0].__class__.process_description
    else:
        dnas[0].__class__.process_description = process_description 
    ...
    if __direct == 1:
        ...
        process_id = make_processid(construct, building_history, process_id)
        add_history(construct, [building_history, "topology: {}".format(topology), ",".join([process_id] + originals)]) 
    else:
...
def modifyends(dna, left="", right="", add=0, add_right=0, add_left=0, project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1):
    project             = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description
    ...
    if __direct == 1:
        ...
        process_id = make_processid(new_dna, building_history, process_id)
        add_history(new_dna, [building_history, "left: {}; right: {}; leftobj: {}; rightobj: {}".format(*ends, args[2], args[3]), ",".join([process_id] + originals)]) 
    else:
...
def flipdna(dna, project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1):
    project             = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description
    ...
    if __direct == 1: 
        ...
        process_id = make_processid(comp, building_history, process_id)
        add_history(comp, [building_history,"", ",".join([process_id] + originals)]) 
    else:
...
def editsequence(dna, source_sequence, destination_sequence, start=0, end=None, strand=1, project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1):
    project             = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description
    ...
    if __direct == 1:    
        ...
        new_dna._product_id = new_dna._unique_id if product is None else product 
        if start == 0 and end == len(dna.seq):
            building_history     = "QUEEN.dna_dict['{}'] = editsequence(QUEEN.dna_dict['{}'], query='{}', source_sequence={}, destination_sequence={}, strand={}{}{}{}{})".format(new_dna._product_id, original_id, query, fsource, destination_sequence, strand, project, fproduct, process_name, process_description)
        else:  
            building_history     = "QUEEN.dna_dict['{}'] = editsequence(QUEEN.dna_dict['{}'], query='{}', source_sequence={}, destination_sequence={}, start={}, end={}, strand={}{}{}{}{})".format(new_dna._product_id, original_id, query, fsource, destination_sequence, start, end, strand, project, fproduct, process_name, process_description)
        if len(history_features) > 1:
            history_feature = _combine_history(new_dna, history_features) 
            new_dna.history_feature = history_feature
        process_id = make_processid(new_dna, building_history, process_id)
        add_history(new_dna, [building_history, "source: {}; destination: {}; start: {}; end: {}; strand: {}".format(source_sequence, destination_sequence, start, end, strand), ",".join([process_id] + originals)])  
...
def editfeature(dna, key_attribute="all", query=".+", source=None, start=0, end=None, strand=2, target_attribute=None, operation=None, new_copy=True, project=None, product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, originals=[], __direct=1): 
    project = project if product is None else product
    process_name        = pn if process_name is None else process_name
    process_description = pd if process_description is None else process_description
    ...
    if operation is None or target_attribute is None:
        ...
        if __direct == 1 and new_copy == True:
            ...
            process_id = make_processid(dna, building_history, process_id)
            add_history(dna, [building_history, "key_attribute: {}; query: {}; start: {}; end: {}; strand: {}; target_attribute: {}; operation: {}".format(key_attribute, fquery, start, end, strand, target_attribute, command), process_id, ",".join([process_id] + originals)])
            if product is None:
                pass 
            else:
...
def visualizedna(dna, map_view="linear", feature_list=None, start=0, end=None, width_scale="auto", height_scale=1.0, label_location=None, linebreak=None, seq=False, label_visible=2, fontsize=None, diamater_scale=1.0, title_visible=True, axis_visible=True, tick_space="auto", labelcolor="k", title=None):
