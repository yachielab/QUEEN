...
From ver 1.1, 
- `quianable` parameter is added for the quinable function.  
- `set_feature` parameter is added for the operational functions excluding `cutdna`.  
  It help users add annotations for the QUEEN object generated by the operational 
  functions.  
...
setfeature : False, dict, list of dict, default: False  
    ...
    object is composed of key-value pairs of DNAfeature attributes. If the parameter 
    is specfied, the DNAfeature objects generated based on the dictionary values would 
    be added in the `.dnafeatures` of newly generated QUEEN objects.  
    ...
    If `None`, any operation would not happen.  
    - `feature_id` : `str`,
...
def joindna(*dnas, topology="linear", setfeature=False, stickyend_length=0, product=None, process_name=None, process_description=None, 
    ...
    compatible region whose length is larger than `stickyend_length`.  
    ...
    stickyend_length : int, default: 0 
        ...
        If the compatible end length is shorter than this value, 'joindna' operation 
        would be interrupted and raise the error message. 
    setfeature : list of dict
        For detailes, see `QUEEN.queen.qobj.__doc__`.
    process_name : str
        For detailes, see `QUEEN.queen.qobj.__doc__`.
