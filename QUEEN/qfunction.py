...
QUEEN objects can be manipulated by four simple operational functions, `cutdna()`, `modifyends()`, `flipdna()`, and `joindna()`, 
that can collectively represent any of the standard molecular DNA cloning processes, 
and two super functions, `editsequence()` and `editfeature()`.

DNA construction process achieved by `QUEEN()` for genearating QUEEN object, the search functions `searchsequence()` and `searchfeature()`, 
operational functions `cutdna()`, `cropdna()`, `modifyends()`, `flipdna()`, and `joindna()` 
and super functions `editsequence()` and `editfeature()` described can progressively be recorded into the manipulating QUEEN object, 
which enables to generate a quine code that replicates the same QUEEN object by the `quine()`.  
From here, we call these functions "quinable" functions.

In addition to the unique parameters and options for the quinable functions, all of them can commonly take the four parameters, 
The `process_name`, `process_description`, and `product` parameters enable annotation and structured visualization of the construction process. 
`setfeature` parameter help users add annotations for the QUEEN object generated by the operational functions.

These optional parameters do not affect the behavior of operations for QUEEN object in quinable functions.

Common parameters
-----------------
process_name (or pn): str , default: ""
    This option enables users to provide label names for process flow groups. An experimental flow composed of sequential operations 
    by quinable functions can be grouped and labeled with a user-defined name by providing the same name to the quinable function operations 
    belonging to the same target group. Such group labels can be, for example, `"PCR 1"`, `"EcoRI digestion"`, `"Gibson Assembly"`, etc. 
    `visualizeflow()` described below takes into account the group information to generate experimental flow maps from `QUEEN_objects`.

process_description (or pd): str, default: ""
    Similar to `process_name`, this option enables users to provide narrative descriptions of operations conferred by quinable functions. 
    This enables the generation of the whole "Materials and Methods" description for a DNA construction process along with 
    its DNA construction flow from a `QUEEN_object` (or a QUEEN-generated GenBank file) using the `quine()` function. 
...
setfeature: False, dict, list of dict, default: False
    This option can be acceptable by only `QUEEN()` and basic operational fuctions `cutdna()`, `cropdna()`, `modifyends()`, `flipdna()` and `joindna()`. 
    A `dict` object is composed of key-value pairs of DNAfeature attributes. If the parameter is specfied, the DNAfeature objects generated based on the 
    dictionary values would be added in the `.dnafeatures` of newly generated QUEEN objects. The following attributes have default values, 
    so if they are not specified in a `dict` object, the values would be set with the default values. If `None`, any operation would not happen.
...
quinable:`bool` (`True` or `False`), default: `True` 
    If `False`, quinable function will not be quinalble, It means that the operation process in the quinable function will not be recorded into the building history.
...
def cutdna(dna, *cutsites, crop=False, setfeature=False, product=None, process_name=None, process_description=None, 
           pn=None, pd=None, process_id=None, original_ids=[], _sourcefile=None, quinable=True, __direct=1):   
    
    """Cut QUEEN object at queried sites.

    Cut `QUEEN_object` at queried positions or by queried `DNAfeature_object` and return a list of fragmented `QUEEN_object`. 
    Each existing `DNAfeature_object` in the original `QUEEN_object` will be inherited to the generating `QUEEN_object`. 
    Suppose any `DNAfeature_objects` are at the cut boundaries being split into fragments. In that case, each `DNAfeature_object` 
    will also be carried over to the new `QUEEN_object` with the `"qualifiers:broken_feature"` attribute to be 
    
    ```
    "[.project of the original QUEEN_object]:[.feature_id of the original DNAfeature_object]:[sequence length of the original DNAfeature_object]:
    [sequence of the original DNAfeature_object]:[start..end positions of the original DNAfeature_object in the sequence of the original QUEEN_object]:
    ...
    cutsites: list of int, str ("int/int"), or/and QUEEN.qobj.DNAfeature object
        List of cut positions. For blunt-end cut, a cut position can be provided by `int`. For sticky-end cut, a cut position can be specified 
        by `"int/int"`, where the left and right integers represent cut positions on the top and bottom strands, respectively. 
        `DNAfeature_objects` holding `"qualifiers:cut_site"` attributes can also be provided to cut a query DNA. This operation cannot proceed 
        with multiple cut sites where a nicking or blunt-end cut of a cutting event happens between two nick positions of another sticky-end cut.
    ...
    if __direct == 1 and quinable == True:
        products = []
...
def cropdna(dna, start=0, end=None, setfeature=False, product=None, process_description=None, process_name=None, 
            pd=None, pn=None, process_id=None, original_ids=[], _sourcefile=None, quinable=True, __direct=1):
    ...
    start: int, str ("int/int"), or QUEEN.qobj.DNAfeature, default: 0
        ...
        When the value is specified by `int` or `"int/int"`, the numerical values should follow the zero-based indexing. 
        ...
        When the value is specified by `int` or `"int/int"`, the numerical values should follow the zero-based indexing. 
        If the topology is `"circular"` and the `start` position is larger than the `end` position, 
        the fragment across the zero position will be returned.
    ...
    subdna, crop_positions = cutdna(dna, start, end, product=project, crop=True, __direct=0)  
    if __direct == 1 and quinable == True:   
        args = []
        new_positions = [] 
...
def joindna(*dnas, topology="linear", setfeature=False, stickyend_length=0, product=None, process_name=None, process_description=None, 
            pn=None, pd=None, process_id=None, original_ids=[], _sourcefile=None, quinable=True, __direct=1):
    """ Assemble `QUEEN_object`. 

    Assemble `QUEEN_object`. The connecting DNA end structures must include compatible region whose length is larger than `stickyend_length`. 
    If the assembly restores unfragmented sequences of `DNAfeature_objects` that are fragmented before the assembly and hold `
    "qualifiers:broken_feature"` attributes, the original `DNAfeature_objects` will be restored in the output `QUEEN_object` 
    (the fragmented `DNAfeature_objects` will not be inherited). 
    ...
    stickyend_length: int, default: 0 
        Minimum compatible sticky end legnth to be required in the assembly. 
        If the compatible end length is shorter than this value, 'joindna' operation would be interrupted and raise the error message. 
...
def modifyends(dna, left="", right="", add=0, add_right=0, add_left=0, setfeature=False, product=None, process_name=None, 
               process_description=None, pn=None, pd=None, process_id=None, original_ids=[], _sourcefile=None, quinable=True, __direct=1):
    """Modify sequence end structures of `QUEEN_object`.

    Modify sequence end structures of `QUEEN_object` according to the specified end sequence structure.
    If the topology is `"circular"` or `"ssdna"`, it won't work.
   
    Notes
    -----
    Sticky ends can be generated by trimming nucleotides where their end structures are given by 
    top and bottom strand strings with `"*"` and `"-"` separated by `"/"`, respectively. 
    The letters `"-"` indicate nucleotide letters to be trimmed, and the letters `"*"` indicate ones to remain.  
    ...
    left: str ("str" or "str/str"),  default: None
        Left sequence end structure of `QUEEN_object`. Please refer to the "Notes" for the specifiction.

    right: str ("str" or "str/str"),  default: None
        Right sequence end structure of `QUEEN_object`. Please refer to the "Notes" for the specifiction.
...
def flipdna(dna, setfeature=False, product=None, process_name=None, process_description=None, 
            pn=None, pd=None, process_id=None, original_ids=[], _sourcefile=None, quinable=True, __direct=1):
    """Invert `QUEEN_object`.
...
def editsequence(dna, source_sequence, destination_sequence=None, start=0, end=None, strand=1, product=None, process_name=None, 
    ...
    Edit sequence of `QUEEN_object` by searching target sequence fragments matched to a `source_sequence` and 
    replacing each of them with a `destination_sequence`. All `DNAfeature_objects` located on the edited sequence 
    regions will be given the `"qualifiers:broken-feature"` attribute. In any sequence edit that confers change 
    in the sequence length of the `QUEEN object`, the coordinates of all affected `DNAfeature_objects` will be adjusted. 
    This is the parental function of `searchsequence()`. If `destination_sequence` is not provided, 
    it works just as `searchsequence()`.
    ...
    source_sequence: regex or str, default: ".+"
        Source sequence(s) to be replaced. If the value is not provided, the entire `QUEEN_object` sequence will be 
        replaced with a `destination_sequence`. It allows fuzzy matching and regular expression. 
        For details, see [https://pypi.org/project/regex/](https://pypi.org/project/regex/). 
        All IUPAC nucleotide symbols can also be used. Substrings of the `regex` value can be isolated by enclosing 
        them in parentheses. Each pair of parentheses is indexed sequentially by numbers from left to right. 
        Isolated substrings can be replaced at once by providing a `destination_sequence` where each substring replacement 
        is designated, referring to the index numbers. For details, see https://docs.python.org/3/library/re.html#re.sub
    ...
    Notes
    -------
    If `destination_sequence` is not provided, it will act as `searchsequence()` and return a `list` of `DNAFeature_objects`. 
...
def editfeature(dna, key_attribute="all", query=".+", source=None, start=0, end=None, strand=2, target_attribute=None, operation=None, new_copy=True, 
                product=None, process_name=None, process_description=None, pn=None, pd=None, process_id=None, original_ids=[], _sourcefile=None, quinable=None, __direct=1):
    """ Search for `DNAfeature_objects`. 
    
    Search for `DNAfeature_objects` holding a `query` value in a designated `key_attribute` and edit a `target_attribute` of 
    the same `DNAfeature_objects` with one of the three operations: `removeattribute`, `replaceattribute`, or `createattribute`. 
    This is the parental function of `searchfeature()`. If `target_attribute` is not provided, it works just as `searchfeature()`.
    ...
    key_attribute: str, default: "all"
        Attribute type to be searched (`feature_id`, `feature_type`, `"qualifiers:*"`, or `sequence`). 
        If the value is not provided, it will be applied to all of the attributes in the `QUEEN_object`, excluding `sequence`. 
        However, if the `query` value is provided with only the four nucleotide letters (A, T, G, and C), this value will be 
        automatically set to `sequence`.    
    
    query: regex or str, default: ".+"
        Query term. `DNAfeature_objects` that have a value matches to the query value for `key_attribute` designated above 
        will be subjected to the edit. It allows fuzzy matching and regular expression.
        For details, see https://pypi.org/project/regex/](https://pypi.org/project/regex/. 
    ...
    source: list of QUEEN.qobj.DNAfearture objects, default: QUEEN_object.dnafeatures
        Source `DNAfeature_objects` to be searched for the editing. `DNAfeature_objects` outside the search range defined by 
        `start`, `end`, and `strand` will be removed from the source. Any `DNAfeature_objects` can be provided here. 
        For example, a list of `DNAfeature_objects`returned from `searchsequence()` or `searchfeature()` operation 
    ...
    strand: int (1 (top strand only), -1 (bottom strand only), and 2 (both strand)1), default: 2
        Sequence strand to be searched.
    
    target_attribute: str, default: None
        Attribute type of the target `DNAfeature_objects` to be edited 
        (`feature_id`, `feature_type`, `"qualifiers:*"`, `strand`, `start`, `end` or `sequence`). 
        If the value is not provided, this will work just as `searchfeature()`.  

    operation: removeattribute(), createattribute(value) or replaceattribute(source_value, destination_value), default: None
        If the operation is not specified, this will work just as `searchfeature()`. 
        
        - `removeattribute()`: This removes `target_attribute` from the target `DNAfeature_objects` but only for 
        `feature_id` or `"qualifiers:*"`. If `target_attribute` is `feature_id`, the entire `DNAfeature_objects` 
        will be erased from the `QUEEN_object`.
        
        - `createattribute(value="str")`: This creates or overwrites target_attributes of the target`DNAfeature_objects` 
        with `"str"`. If `target_attribute` is `feature_id` and there is no existing  `DNAfeature_object` with the same 
        `feature_id` of `"str"`, it will create the new `DNAfeature_object` in the `QUEEN_object.dnafeatures`. 
        If the search by `DNAfeature_objects` determines multiple `DNAfeature_objects` to be created, each `feature_id` of 
        the new `DNAfeature_objects` is generated as `"str-number"`, where `numbers` follow the order they were searched. 
        If the same `feature_id` of `"str"` already exists in the operating `QUEEN_object.dnafeatures`, the `DNAfeature_object` 
        will be generated with the `feature_id="str-number"`. If `target_attribute` is `"qualifiers:*"`, the qualifier whose 
        value is `"str"` will be added into the `.qualifiers` of the target `DNAfeature_object` as long as it does not overlap 
        with the existing `.qualifiers`.

        - `replaceattribute(source_value=regex or str, destination_value=str or int)`: This will search for substrings 
        in values of the target_attributes of the target `DNAfeature_object` that match to the `source_value` and replace 
        them with the `destination_value`. Similar to `editsequence()`, substrings of the `regex` value can be isolated 
        by enclosing them in parentheses. Each pair of parentheses is indexed sequentially by numbers from left to right. 
        Isolated substrings can be replaced at once by providing a `destination_sequence` where each substring replacement 
        is designated, referring to the index numbers. For details, see https://docs.python.org/3/library/re.html#re.sub. 
        If the `target_attribute` is `sequence`, the sequences corresponding to the target `DNAfeature_object` can be 
        modified like `editsequence()`. When the `source_value` is not provided, the entire data value will be replaced 
        with the `destination value`. If the `target_attribute` is `feature_id`, the replacement will be operated only 
        when no conflict with the existing `DNAfeature_object`. If `target_attribute` is `start`, `end`, or `strand`, 
        no `source_value` is required, and the `destination_value` must be `int`.
    
    new_copy: bool, default: True
        If `True`, it will first generate a copy of the `QUEEN_object` and edit it. Otherwise, the original `QUEEN_object` will be 
        edited directly (Note that this mode does not record the operation process into the building history).
    ...
    if operation is None or target_attribute is None:
        ...
        if __direct == 1 and new_copy == True and quinable == True:
...
def visualizemap(dna, map_view="linear", feature_list=None, start=0, end=None,label_location=None, display_label=2, display_title=True, display_axis=True, fontsize=None, 
    ...
    Generate annotated sequence map of `QUEEN_object` with selected `DNAfeature_objects`. Each feature annotation label is retrieved from 
    the `"qualifier:label"` attribute. All feature annotations and their label Locations of feature annotation labels are automatically 
    adjusted to prevent overlaps on the sequence map. The face color and edge color of each feature annotation are also automatically 
    assigned from the default colormap. However, they can be determined by `"qualifiers:edgecolor_queen"` and `"qualifiers:facecolor_queen"` 
    ...
    feature_list: list of `DNAfeaure_objects`, default: `QUEEN_object.dnafeatures` excluding those with 
    the feature type `"source"`. DNAfeature_objects to be displayed on the sequence map. 
    
    fontsize: int, default: 12 for "circular" map and 10 for "linear" map
        Common font size. Separate font sizes can also be defined for different `DNAfeaure_objects` by editing 
        the `"qualifiers:fontsize_queen"` attribute, which overrides the common font size. 
    
    labelcolor: str, default: "black"
        Common font color for all feature labels. Separate font colors can also be defined for different `DNAfeaure_objects` 
        by editing the `"qualifiers:labelcolor_queen"` attribute, which overrides the common font color.
    
    display_label: 0, 1, or 2, default: 2
        If `2`, all of the labels will be displayed. If `1`, only the feature labels that can fit inside the object boxes 
        will be displayed. If `0`, feature labels won't be displayed.
    ...
    start: int (zero-based indexing),  default: 0, The parameter is available for only linear maps.  
        Start position of the `QUEEN_object` sequence to be displayed. 

    end: int (zero-based indexing),  default: the last sequence position of `QUEEN_object`, The parameter is available for only linear maps.  
    ...
    label_location: float, default: "either"` when `seq` is `False`, otherwise "top"), The parameter is available for only linear maps.
        Feature label locations. Each feature label is generally placed inside the object box. However, if a feature label is larger than 
        the object box, the label will be put outside. If this value is `"either"`, labels will be put below or above the object boxes, 
        whichever is available. If this value is `"top"`, labels will be put above the object boxes. 
        If `seq` is `True`, the value must be set to `"top"`.
    
    linebreak: int or None, default: Length of the `QUEEN_object` sequence, The parameter is available for only linear maps.
    ...
    standard_scale = 4000
    if map_view == "circular":
        fig, axes = vc.visualize(dna, format=0, feature_list=feature_list, bottom=400 * diamater_scale, label_visible=display_label, fontsize=fontsize, 
