...
class DNAfeature(SeqFeature):
    ...
    def set_position(self, position, attribute):
        ...
        if type(position[0]) in (Qint, int) and type(position[1]) in (Qint, int):
            s = position[0]
            e = position[1] 
            if s < e:
...
class QUEEN():    
    """Represent an annotated (double- or single-stranded) DNA object.

    A `QUEEN` instance encapsulates a DNA sequence, its topology
    (linear or circular), sequence annotations (`DNAfeature` objects),
    and a construction history that records how the object was generated
    (e.g., PCR, digestion, ligation).  Sequences can be created from raw
    strings or imported from files or remote databases.

    Typically, you will create `QUEEN` objects either by providing a DNA
    sequence string via ``seq`` or by specifying a file path or identifier
    via ``record`` and ``fileformat``/``dbtype``.

    Parameters
    ----------
    seq : str, optional
        DNA sequence (5′→3′ top strand) used to construct the `QUEEN`
        object. Mutually exclusive with ``record`` in typical use.
    record : str, optional
        Source from which to load the sequence and annotations. This may
        be a local GenBank/FASTA file path, an NCBI accession, a plasmid
        identifier in an external repository, or a shared link from an
        online sequence editor, depending on ``dbtype``.
    fileformat : {"genbank", "fasta"}, optional
        Explicit file format for ``record`` when reading a local file.
        If ``None``, the format is inferred from the file extension where
        possible.
    dbtype : {"local", "ncbi", "addgene", "benchling", "googledrive"}, optional
        How to interpret ``record`` when it is not a local file path.
    ...
    topology : {"linear", "circular"}, optional
        Sequence topology. Default is ``"linear"``. When loading from a
        GenBank file, the topology is inferred from the file if possible.
    ssdna : bool, optional
        If ``True``, the `QUEEN` object represents single‑stranded DNA.
        Default is ``False``.
    import_history : bool, optional
        When loading from an existing GenBank file that already contains
        construction history annotations, control whether that history is
        imported into the new `QUEEN` object. Default is ``True``.
    supfeature : bool, optional
        If ``True``, supplementary features (for example, utility
        annotations generated by other tools) may be imported or generated
        on load. Exact behavior depends on the implementation. Default is
        ``False``.
    project : str, optional
        Project or construct name. If omitted, a unique name is generated
        based on the sequence or ``product``. Also used as a key in
        internal dictionaries (e.g., ``QUEEN.dna_dict``).
    product : str, optional
        Human‑readable name of the construct. Often recorded in the
        construction history and used as a label in downstream functions.
    process_name : str, optional
        Name of the process that created this `QUEEN` object (for example,
        ``"PCR"`` or ``"Ligation"``). Typically set by higher‑level
        helper functions (e.g., in :mod:`qexperiment`).
    process_description : str, optional
        Free‑text description of how the object was constructed.
    pn : str, optional
    ...
    process_id : str or int, optional
        Identifier for this construction step in the global history.
    original_ids : list, optional
        List of identifiers of input `QUEEN` objects from which this
        object was derived (used internally to track provenance).
    quinable : bool, optional
        If ``True`` (default), operations on this `QUEEN` object are
        recorded such that :func:`quine` can reproduce the same object
        programmatically.
    **kwargs
        Additional implementation‑specific options (for example, arguments
        forwarded to the underlying parsers when reading files).

    Attributes
    ----------
    project : str
        Project name / ID for this construct. Often used as a key in
        ``QUEEN.dna_dict``.
    seq : str
        Top‑strand DNA sequence (5′→3′). This attribute is not meant to be
        edited directly; sequence edits should be performed via the
        operational functions (:func:`cutdna`, :func:`modifyends`,
        :func:`joindna`, etc.).
    rcseq : str
        Bottom‑strand DNA sequence (5′→3′), i.e. the reverse complement of
        the top strand. Generally managed automatically.
    topology : {"linear", "circular"}
        Sequence topology.
    dnafeatures : list of DNAfeature
        Sequence annotations (genes, promoters, primers, etc.). Each
        `DNAfeature` has a type, location, strand, and qualifiers.
    history : dict
        Construction history mapping process IDs to the expression string
        (and arguments) that produced this `QUEEN` object, used by
        :func:`quine` and by reporting utilities such as
        :meth:`QUEEN.printprotocol` and :meth:`QUEEN.outputgbk`.

    Notes
    -----
    Most users do not instantiate `QUEEN` directly with all parameters.
    Instead, start from a simple constructor and then operate with higher‑
    level functions:

    * Import from a local GenBank file:

      ``dna = QUEEN(record="path/to/file.gb", fileformat="genbank")``

    * Build from a raw sequence:

      ``dna = QUEEN(seq="ATGC...")``

    Subsequent in silico cloning steps (PCR, digestion, ligation, etc.) are
    typically performed using functions from :mod:`qexperiment` or
    :mod:`qfunction`, which in turn create new `QUEEN` objects and update
    ...
    def _get_genbank(_id, dbtype="ncbi"):
        ...
        if dbtype == "ncbi":
            ...
            _id = re.match(r"https://benchling\.com/s/seq-[A-Za-z0-9]+", _id).group() 
            url = _id + ".gb"
            ...
            match = re.search(r"https://drive\.google\.com/file/d/(.+)/view\?usp=sharing", url)
    ...
    def __init__(self, seq=None, record=None, fileformat=None, dbtype="local", topology="linear", ssdna=False, import_history=True, supfeature=False, project=None, product=None, process_name=None, process_description=None, 
        ...
        project : str
            ...
            `.productdict` keys.
        seq : str  
            Top strand sequence (5'-to-3'). This property cannot be directly edited; only the 
            built-in operational functions of QUEEN described below can edit this property.
        rcseq : str  
            Bottom strand sequence (5'-to-3'). This property cannot be directly edited; only the 
            built-in operational functions of QUEEN described below can edit this property.
        topology : str ("linear" or "circular")   
                    ...
                    if feat.type == "source" and feat.start == 0 and feat.end == len(self.seq):
                        for key in feat.qualifiers:
                            if "building_history" in key[0:18] and import_history == True:
                                ...
                                results = re.findall(r"QUEEN.dna_dict\['[^\[\]]+'\]", history) 
    ...
    def printsequence(self, start=None, end=None, strand=2, hide_middle=None, linebreak=None, display=True):
        """Print and/or return a portion of the sequence, including end structures.

        This method formats the `QUEEN` sequence (top and/or bottom strand),
        optionally compressing the middle of long molecules while preserving
        end structures (cohesive or blunt ends). It can either print to
        standard output or just return the sequence as a `Qseq` object.

        Parameters
        ----------
        start : int or None, optional
            Zero‑based start index of the region to display. If ``None``,
            the beginning of the sequence is used (index ``0``). For circular
            sequences, indices may wrap around.
        end : int or None, optional
            Zero‑based end index (exclusive) of the region to display.
            If ``None``, the end of the sequence is used. For circular
            sequences, indices may wrap around.
        strand : {1, -1, 2}, optional
            Which strand(s) to display and return:

            * ``1`` – top strand only (5′→3′).
            * ``-1`` – bottom strand only (5′→3′, i.e. reverse complement).
            * ``2`` – both strands, separated by ``"/"`` in the returned
              `Qseq`. Default is ``2``.

        hide_middle : int or None, optional
            For long sequences, controls how many bases at each end of the
            selected region are shown before compressing the middle with a
            placeholder. If ``None``, the default is:

            * ``None`` if ``len(self.seq) < 100`` (no compression).
            * ``20`` otherwise.

        linebreak : int or None, optional
            Line width in characters for printing. If ``None``, the entire
            selected region is printed in a single line. This parameter
            affects only formatted output, not the returned sequence.
        display : bool, optional
            If ``True`` (default), the formatted sequence is printed to
            standard output. If ``False``, no printing is performed.

        Returns
        -------
        Qseq
            A `Qseq` object containing the selected sequence. For ``strand==2``,
            the top and bottom strands are concatenated with a ``"/"``
            separator. Metadata such as name are set to reflect the region and
            strand selection.

        Raises
        ------
        ValueError
            If ``start`` or ``end`` is negative, or if both ``start`` and
            ``end`` are not ``None`` and the resulting region cannot be
            interpreted on the sequence (e.g., invalid wrap in linear
            topology).

        Examples
        --------
        Print and return the full sequence of a circular construct::

            seq_view = dna.printsequence()

        Return only the top strand for a subregion without printing::

            seq_view = dna.printsequence(start=100, end=200, strand=1, display=False)
        """
    ...
    def setfeature(self, attribute_dict=None):
        ...
        if "feature_id" not in attribute_dict:
            ...
            feature_ids.sort(key=lambda x: (features_dict[x].start, -1 * features_dict[x].end))
            starts = [("-1", 0)] + [(_id, features_dict[_id].start) for _id in feature_ids]
            for _id, start in starts:
    ...
    def printfeature(self, feature_list=None, attribute=None, separation=None, seq=False, output=None, display=True, x_based_index=0, return_df=True):
        ...
        format with selectable attributes. The table can be printed to
        standard output, written to a file, and/or returned as a
        :class:`pandas.DataFrame`.

        Parameters
        ----------
        feature_list : list of DNAfeature or None, optional
            List of features to include. If ``None`` (default), all features
            in ``self.dnafeatures`` are used (excluding internal history
            features, if filtered by implementation).
        attribute : list of str or "all" or None, optional
            List of feature attribute names to include as columns. Default is
            ``["feature_id", "feature_type", "qualifier:label", "start",
            "end", "strand"]``.  If ``"all"``, all available attributes except
            raw sequence fields are included.
        separation : str or None, optional
            Column separator string used when printing rows. If ``None``,
            spacing is automatically adjusted for readability. If ``","``,
            a CSV‑style output is produced.
        seq : bool, optional
            If ``True``, the sequence corresponding to each feature (in its
            encoded direction) is included as an additional column.
        output : str or IO or bool or None, optional
            Output destination. If a string, it is treated as a file path and
            opened for writing. If an open file‑like object, rows are written
            to that stream. If ``False``, no output is written. If ``None``
            (default) and ``display=True``, output is printed to standard
            output.
        display : bool, optional
            If ``True`` (default), write the table to the destination given by
            ``output`` (standard output by default). If ``False``, no printing
            is performed.
        x_based_index : {0, 1}, optional
            Coordinate system for feature positions. ``0`` (default) uses
            zero‑based indexing (Python/QUEEN style). ``1`` uses one‑based
            indexing (as in GenBank files).
        ...
        pandas.DataFrame or None
            A DataFrame with one row per feature and one column per selected
            attribute (plus additional columns such as sequence when
            requested), or ``None`` if ``return_df=False``.

        Raises
        ------
        ValueError
            If an attribute name in ``attribute`` is not recognized for any
            feature and cannot be resolved.
        IOError
            If writing to ``output`` fails.

        Examples
        --------
        Display the default feature table in the console::

            df = dna.printfeature()

        Export feature annotations to a CSV file::

            df = dna.printfeature(
                attribute=["feature_id", "feature_type", "qualifier:label"],
                separation=",",
                output="features.csv"
            )
        """
    ...
    def printcutsite(self, site="single", display=True):
        """Print a tidy table of digestion sites for this `QUEEN` object.
        """
        cutters = []
    ...
    def printprotocol(self, execution=False, output=None):
        """Print a human-readable protocol derived from the construction history.

        This method delegates to the global :func:`printprotocol` utility
        (typically imported from the `quine` or related module) to render the
        sequence of qexperiment operations that produced this `QUEEN` object
        as a human‑readable protocol or script.

        Parameters
        ----------
        execution : bool, optional
            If ``True``, include execution‑ready commands (for example,
            code snippets) suitable for reproducing the construction.
            If ``False`` (default), produce a more narrative protocol
            description.
        output : str or IO or None, optional
            Output destination. If a string, treated as a file path. If an
            open file‑like object, protocol text is written to that stream.
            If ``None`` (default), the protocol is printed to standard output.

        Returns
        -------
        str or None
            A string containing the protocol text if the underlying
            :func:`printprotocol` returns one, or ``None`` if no value is
        ...
        Examples
        --------
        Print a protocol describing all qexperiment steps applied to
        this construct::

            dna.printprotocol()

        Export an execution‑oriented script to a text file::

            dna.printprotocol(execution=True, output="protocol.txt")
        """
    ...
    def outputgbk(self, output=None, format="genbank", record_id=None, annotation=None, export_history=1, describe_brokenfeature=True, qexperiment_only=True, _return=False):
        """Export the `QUEEN` object to GenBank (or related formats).

        This method writes the current `QUEEN` object as a
        :class:`Bio.SeqRecord` to a GenBank‑style file or to standard output.
        In addition to ordinary sequence features, it can embed a construction
        history (building history) as a dedicated feature, depending on the
        ``export_history`` option.

        Parameters
        ----------
        output : str or IO or None, optional
            Output destination. If a string, it is treated as a file path to
            write. If an open file‑like object, the record is written to it.
            If ``None`` (default), the record is written to standard output.
        format : {"genbank", "fasta", ...}, optional
            Output format string passed to :mod:`Bio.SeqIO`. Default is
            ``"genbank"``.
        record_id : str or None, optional
            ``id`` of the exported :class:`SeqRecord`. If ``None``, the
            object's project name is used.
        annotation : dict or None, optional
            Additional annotations to set on the :class:`SeqRecord` (for
            example, ``{"molecule_type": "DNA"}``). See the Biopython
            documentation for allowed keys.
        export_history : {0, 1, 2}, optional
            How to export construction history:

            * ``0`` – do not include any construction history features.
            * ``1`` (default) – include only the history of the current
              `QUEEN` object (qexperiment commands).
            * ``2`` – include full history for the current object and all
              ancestor objects, if available.

        describe_brokenfeature : bool, optional
            If ``False``, remove all "broken feature" qualifiers from the
            exported features. Default is ``True``.
        qexperiment_only : bool, optional
            If ``True`` (default), only qexperiment‑level commands are
            exported in the history feature(s). Internal low‑level commands
            used to implement qexperiment steps are omitted.
        _return : bool, optional
            If ``True`` and ``output`` is ``None`` (standard output), return
            the generated text as a single string instead of printing it.
            For file outputs, this flag has no effect and ``None`` is
            returned. Default is ``False``.

        Returns
        -------
        str or None
            When ``_return=True`` and ``output`` is ``None``, the GenBank (or
            other chosen) representation is returned as a string. Otherwise,
            ``None`` is returned after writing.

        Raises
        ------
        ValueError
            If ``export_history`` is not one of ``0``, ``1``, or ``2``.
        IOError
            If writing to the output destination fails.

        Examples
        --------
        Write a GenBank file to disk::

            dna.outputgbk(output="construct.gb")

        Obtain a GenBank string instead of writing to disk::

            gb_text = dna.outputgbk(output=None, _return=True)
        """
