...
def map_feat(fig, ax, ax2, feats, length, head_length=np.pi * 0.02, unvisible_types=["source"], visible_types=[], enlarge=1.0, format=1, bottom=400):
    ...
    for i, feat in enumerate(feats):
        if feat.type in unvisible_types:
            ...
            if format == 0: 
                ...
                new_origins    = [pos * length / (2*np.pi) for pos in ([ p[0] for p in new_pos_list] + [gs, ge])] 
                new_gs_origin = min(new_origins) 
                new_ge_origin = max(new_origins)
                ...
                new_origins    = [pos * length / (2*np.pi) for pos in ([ p[0] for p in new_pos_list] + [gs, ge])] 
                new_gs_origin = min(new_origins) - 2 * mg * length 
                new_ge_origin = max(new_origins) + 2 * mg * length
    ...
    if format == 0:
        ylim = max(y_list)*lane_h + bottom_h + 240
    else:
        ylim = max([max(y_list)*lane_h + bottom_h, max(ty_list)]) + 240
    if ylim < normal_w:
        ylim = normal_w
